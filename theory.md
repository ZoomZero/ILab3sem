...
Инвариантом RAII класса является то, что никто, кроме методов класса не имеет владеющего доступа к p. Кроме методов класса к закрытым методам класса имеют статистические функции или friend функции. Дружить можно ещё с классами и шаблонами. Все методы получают this, остальные ещё нет.  
...
## Лекция 5
### Операторы. Двумерные массивы. 
RAM модель памяти одномерна. В С row-major order, так как конструкция a[2][3] синтаксически неоднозначная. Она может быть прочиатан как (a[i])[j]. Соотвественно имеем **jagged arrays** - массивы указателей. Отсюда массиво-подобное вычление и указательно-подобное.    
**Массивы гниют изнутри. Можно отпускать только самый вложенный(внешний, наоборот, насколько я понял) индекс**  
Используя a[] как lvalue, рехультат неоднозначен. 
```
extern int *a; //где-то есть настоящая ячейка а
extern int b[]; //где-то есть массив b какой-то длины
```  
Разница в ассемблере:
```
aval = load[a]; i = load[aval+5*sizeof(int)];  
i = load[b+5*sizeof(int)];
```
### Перегрузка типов и операторов
В С++ обязателньо используется один из трёх cast-ов:
```
static_cast - безопасные преобразования. сохраняет значение, но меняет побитовое представление.
const_cast - снимает логическую константность
reinterpret_cast - не меняет побитовое значение. слабоумие и отвага, но лучше, чем C style cast(так как он сделает вообще всё).
```
С++ cast легко видны в коде, но по хорошему лучше обходиться бз них. Но иногда они необходимы. 
### Особенности неявного приведения
Наследство от языка С. type op ftype => ftype op ftype  
При инициализации неянвые cast. Они дествуют и для наших типов. Это не всегда хорошо, допустим при приведении числа к какой-то стройокй.  
**Заблокировать можно с помощью explicit.**  
 Когда мы хотим неявно преоразовать усовно quat<int> в int, надо писать
```
template<typename T> struct Quat {
 T x_ = 0; y_ = 0; 
operator T() {return x_;}  } //или operator int и тд.
```
На такие операторы можно навесить explicit, тогда только явные преобразования.  
**Стандартные преборазования выигрывают у пользовательских.**  
### Positive hack  
```
...
```
operator-(Quat<T> arg) { return Quat<T>{-arg.x, -arg.y};  //унарный минус
```  
Это функция, а можно использовать метод в классе.  
**Для int перегружать ничего не надо, как и для всего.**  
###  Перегрузка цепочечных операторов
Они все правоассоциативны. Возвращать *this, чтобы можно было писать цепочкой. Внутри класса + определять не надо. Лучше отдельной функцией - вне класса. Он сможет работать через +=. Ассоциативность переопределённых операторов такая же, как и у обычных. 
  
## Лекция 6
Должен ли оператор сложения действительно складывать? Согласовывать его с цепочечным +=? 
Нет. Хорошим тоном является поддерживать консистентную семантику.  
Меньше всего повезло оператору сдвига. Он испольузется для ввода-вывода. Обычно он определяется ве калсса, спользуя внутренний dump:
```
template <typename T> struct Quat {
T x, y;
...
template <typename T>
std::ostream& operator<<(std::ostream& os, const Quat<T>& q) {q.dump(os); return os;}
```
Для матриц переопределние весьма тяжелая штука. Будет создаваться довольно дорогой временный объекут(если через цепочку переопределять +).  
### Сравнения как бинарные операторы
```
template <typename T> //сравниваем локации
bool operator== (const Quat<T>& lhs, const Quat<T>& rhs) { return (&lhs == &rhs); }

template <typename T> //сравниваем значения
bool operator== (const Quat<T>& lhs, const Quat<T>& rhs)
{ rerutrn (lhs.x == rhs.x) && (lhs.y == rhs.y); }
```
Оператор равенства должен быть рефлексивным(равенство самому себе), симметричным и транзитивным.  Нужно в комментариях к коду это доказать по-хорошему.  
В С++ приняты двувалентные сравнения. В стандарте 2020 года появился "оператор летающая тарелка":
```
std::strong_ordering operator <=>(const MyInt &rhs) {return x_ <=> rhs.x_ };
```
Таким образом переопределяем все сравнения. Но что он такое? Возвращает один из трёх ordering_type(различимы ли равные значения, и возможны ли несравнимые значения).  
Но == пока не определен. Можно добавить:
```
bool operator== (...) const = default;
//или переписать
auto operator <=>(...) const = default;
```
Для переопределниея пре-инкремента\пост-инкремента используется один и тот же оператор. По сути это преинкремент. Для переопределния постинкремента нужно: operator++(int);  
Обычно постинкремент в терминах преинкремента.
```
...
{ Quat<T> tmp {*this};
  ...
  retrurn tmp : } };
```
Поэтому профессионал использует преикремент в for-цикле. Ещё признаком профи является:
```
for (itt it = cont.begin(), ite = cont.end() it != ite; += it)
```
### Для работы с указателе-подобными обектами можно перегрузить взятие адреса, разыменование и обращение с разыменованием.
Оператор-> зарывается в глубину. У него вызов не как обычно, a.operator(b), а так: (a.operator->())->a.  
Адресс объекта, если нам нужен его действительный физический адрес, можно получить с помощью:
```
std::adressof();
```
### Указатели на методы классов
Метод частично ведёт себя как функция. Есть специальный синтаксис.
```
using constif_t = int(MyClass::*)(flot, int) const;

constif_t ptr = &MyClass::DoIt;
MyClass c; (c.*ptr)1.0, 1);
//или
MyClass * pc = &c; (pc->*ptr)(1.0, 1); //и этот метод даже перегружается. приоритета вообще нет, требований к перегрузке нет.
```
### Индексаторы
```
class MyVector 
...
```
std::sort неэффективно вызывать с кастомным предикатом. Тогда идёт вызов по указателю на этот предикат и всё замедляется. Способы решения:  
1) Функтор - класс, который ведёт себя как функция. Сделаем неявное приведение к указателю на функцию
```
struct gt
{ static bool gtf(int x, in y) {return x>y;}
  using gtfptr_t = bool(*)(int, int);
  operator gtfptr_t() const {return gtf } };
std::sort  (obj, obj+smth, gt{});
```
Но выглядит это так себе и плохо расширяется.  
**2) Перегрузка оператора (). Так же с функтором, но уже stateless.**
```
struct gt
{ bool operator() (int x, int y) { return x > y; } };
std::sort  (obj, obj+smth, gt{});
```
Это проще и так же эффективно.  
  
**ДЗ№4. Спроектировать свой класс матриц. На вход размер n и элементы построчно. Сначала написать под то тест, чтобы понимать, что требуется проектировать. Нам точно нужен консруктор, заполненной значением; конструктор для создания из заданной последовательности; конструктор для создания единичной матрицы; копирующий констурктор; селекторы; деструктор; отрицание; транспонирование; операторы переопределить; инверсия(обратная матрица); оператор равно; вычисление определителя(композицией матриц). Есть проблема - получение провисшей ссылки при возвращаемом временном объекте. Для этого нужно использовать lvalue-qualificator**
```
Matrix& negate() &; //больше не может работать на временных значениях. будет работать только на настоящем объекте.
```
Почитать доклад Бена Дина о пергрузке операторов на CppCon 2018.
## Лекция 7
Любой аборт в программе - плохо.  
Что из матрицы возвращает оператор []? Как лучше сделать - прокси-объекты:
```
...
### Оператор запятая
```
result = foo(), bar(); <=> foo(); result = bar();
```
Переружать нельзя, так как потеряем отношения последования - sequencing.  
```
y = x++ + x++; //unsequenced. UNDEFINED BEHAVIOUR
```
**Перегрудать нельзя:**  
1) Доступ через точку a.b  
2) Доступ к члену класса через точку a.*b  
3) Доступ к пространсву имён a::b  
4) Последовательный доступ a ; b  
5) Почти все специальный операторы, в тч sizeof  
6) static_cast и др.  
7) Тернарный оператор a ? b : c  
**Что не следует перегружать:**
1) Длинные логические операции && и ||  
2) Запятую и унарный плюс  
  
### Наследование
Грамматика представляется синтаксическим деревом. Но как спроектировать узлы? Там же может быть операция или просто число.  
У unionсть проблемы с конструкторами и деструкторами. Надо всё прописывать.  
Можно завести структуру, знающую свой тип. Это ужасно, как минимум из-за конструкции узла бинарного оператора.
**Решение - открытое наследование**  
```
class A {};
class B : public A {}; //B is also A  
```
Поскольку объект производного класса является объектом базового, указаели и ссылки приводятся неявным приведением. Обратно можно через static_cast.  
наследовать можно только тогда, когда В расширяет А и В является частным случаем А.  
### Проблема срезки
```
struct A {
  int a_;
...
```
В итоге поулчим 5 8. Так как конструктор класса А не знает о В (присваивание не полиморфно).  

**ДЗ№5. Hаписать синтаксическое дерево и его интерпретацию языка ParaCL. арифметика+if+while.**

### Виртуальные функции и полиморфизм
Когда хочется для двух классов реализовать один и тот же метод. Пример плохого кода:
```
struct ISquare { virtual double square() const; };
template <typename T> strict Triangle : public ISquare { Point<T> x, y , z; double square() const ; }
```
Просто иллюстрация концепции. Просто совпадение имени означает переопределение (overriding) виртуальной функции, а не перегрузку.  
При создании класса с хотя бы одним виртуальным методом в него добавляется vptr. Конструктор базового класса динамически выделяет память для таблицы виртуальнвх функций. Конструкто каждого потомка производит инициализацию её своими методами.  
Порядок конструирования - сначала базовые подобъекты, а потом поля.  
**Статический и динамический тип**
```
double sum_sq (const ISquare &lhs, const ISquare &rhs) { return lhs.square() + rhx.square(); }
Triangle t; Polygon p;
sum_sq(t, p);  // динамические типы - триангл и полигон, а статические - ISquare
```
Функция sum_sq - полиморфна, так как может иметь разные динамические типы. То есть она динамически полиморфна.  
Имена могут быть не только переопределны, но ещё и перегружены(overloading).  
**Для контролирования этого нужно писать ключевое слово override. Тем самым обозначаем, что имеем в виду переопределние.**  
Функцию надо сделать чисто виртуальной - чтобы она не требовала тела. Объект класса, где есть чисто виртуальный метод - ошибка. 
```
struct ISquare { virtual double square() const = 0; };
```
При удалении по указателю на базовый класс будет утечка памяти. Нужно создавать виртуальный деструктор, чтобы он вёл себя полиморфно. Он служит для уничтожения объектов произвольного класса по указателю на подобъект базового класса.
```
virtual ~ISquare() {}
```
Класс в котором все методы виртальные служит общим интерфейсом. Называется абстрактным базовым классом.  
Чтобы что-то копировать:
```
virtual ISquare *clone() const = 0;
...
Triangle *clone() const override { return new Triangle{pts_}; }
```
Указатель на производный класс то же самое, что и указатель на базовый класс.  
Виртуальное присваивание будет решением проблемы базовой срезки. Хотя проблемы всё равно есть.  
### PVC
Распространённая ошибка - вызов чисто виртуального метода. 
```
struct Base {
  // Base() { doIt(); }
  Base() { safe(); }
...
```
Делая чисто виртуальный деструктор мы рискуем нарваться на это. Обойти можно с помощью: сделать тело виртуального деструктора.  

## Лекция 8
Любая функция внутри класс - inline. А тогда по определению она static.  
Const на reference - хорошая идея при передаче в функцию(но стоит помнить, что это указатель в большинстве случаев), а на value не супер обязательно.  
Тесты можно сделат и юнит(когда разрабатываем), но неплохо было бы попробовать end-to-end тесты.  

Чтобв от класса не могли наследоваться:
```
struct Foo final {...};
```
Теперь попытка наследовать - ошибка компиляции.  
**Чтобы написать класс правильно есть 4 способа:**
1) Класс содержит виртуальный деструктор  
2) Класс объявлен как final  
3) Класс является stateless и подвержен EBCO (empty base class optimizations. пустые классы нужны для так называемых миксинов)     
4) Класс не может быть уничтожен извне, но может быть уничтожен потомком (protected - для защиты от всех, кроме наследников)  
```
class PureBase {
 ...
protected:
...
```
Теперь объект класса-наследника просто нельзя удалить по указателю на базовый класс. 

### Девиртуализация
Ключевое слово final помогает девиртализации. Бранч виртальных функций сложно подсказать, они плохо влияют на производителньость.  
Любой вызов из консруктора - невиртуальный. Отсюда PVC.  
Виртуальные функции связываются динамически(разрешение адреса ункции через vtbl во время выполнения), обычныу функции связываются сатически. Аргументы по умолчанию тоже имеют статическое связывание(то есть зависят только от статического типа).  
```
struct Base {
...
int main() {
  Base *pb = new Derived{};
  std::cout << pb->foo() << std::endl; //printed 28
  delete pb;
}
```
...
```
Приватная функция публично переопределна, и это нормально.  

### Ещё о полиморфизме
Функция полиморфна по какому-то аргументу. Она ведёт себя по-разному в зависимости от типа аргумента.  
Писать лучше прлиморфную функцию на шаблонах, но если нельзя - можно прибегать к виртуальным функциям, то есть к динамическому полиморфизму.  
Теоретически можно было бы сделать шаблонные виртуальные методы. Но тогда бы появились многомерные таблицы виртуальных функций, а это сложно.  
